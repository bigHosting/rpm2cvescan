...
GetOptions( \ my %options,
        ...
        'o|outputfile=s' => \ my $outputfile,
);

if ( (! (defined $inputfile)) || (! (defined $outputfile)) )
{
    die "[*] $0 ERROR: --inputfile and --outputfile must be specified";
}

if ( ! ( -f -r $inputfile) )
{
        die "[*] $0: ERROR: cannot read from $inputfile";
}
...
foreach my $line (<LOG>)
{
        ...
        my ($remotehost, $distro, $packagelist, $packagelist2) = ( split /:::/, $line );
        my %packages_list = map  { split(/\s+/, $_, 2) } grep { m/\s+/ } split(/,/, $packagelist);
        ...
        my %packages_nice = map  { split(/\s+/, $_, 2) } grep { m/\s+/ } split(/,/, $packagelist2);
        %packages_nice = map {
            (my $new = $_) =~ s/\.centos//;
            $new => $packages_nice{$_}
        } keys(%packages_nice);
        ...
        foreach my $key ( sort keys %vulnerable_software )
        {
                ...
                foreach my $cve ( sort (&uniq(@{ $vulnerable_software{$key} })) )
                {
                            ...
                            if ( $csv )
                            {
                                    $csv_output .= sprintf ("VULN,%s,%s,%s,%s,%s,%s,%s\n", $key, $cve, $score, $rhsa, $date, $remotehost, $distro);
                            }
                            ...
                            if ($json)
                            {
                                    ...
                                    $json_data{ScannedHostName} = $remotehost;
                                    ...
                                    $json_data{BusinessUnit}    = "CompanyName";
        ...
        print "\n\n$remotehost TOTAL_UNIQ_PACKAGES=" . scalar(@packages_installed) . ", AFFECTED_PACKAGES=" . $counter_pkg . " CVEs=" . $counter_cve . " HIGHRISK=" . $counter_highrisk . "\n\n";
